...
#### ✅ 清倉特價頁面修正（/clearance）
- **問題**：`/clearance` 頁面同樣使用硬編碼假資料（Nike Air Max 90、Adidas Superstar 等虛假產品）
- **解決方案**：
  - 改用 `GET_HOMEPAGE_PRODUCTS` GraphQL 查詢獲取真實產品資料
  - 自動過濾折扣 >= 30% 的產品作為清倉商品
  - 支援折扣範圍篩選（5-6折、4-5折、3-4折、3折以下）
  - 支援多種排序（折扣最多、價格低到高、價格高到低、銷量優先）
  - 動態計算最高折扣和清倉商品總數
  - 根據折扣幅度和庫存自動決定清倉原因標籤（最後出清、換季出清、斷碼特賣等）
  - 低庫存（<= 10 件）自動顯示「最後庫存」警告
- **技術變更**：`app/clearance/page.tsx`
  - 移除所有硬編碼假資料和尺碼篩選（因產品資料結構不同）
  - 新增 `ClearanceProduct` 介面定義
  - 新增動態統計數據計算
  - 新增載入中、空資料、錯誤狀態處理
...
#### ✅ 搜尋功能擴展 - 支援分類與品牌搜尋 + 中文拆字搜尋
- **問題描述**：搜尋使用精確子字串匹配（`contains`），導致中文搜尋結果極少。例如搜「女鞋」只匹配名稱中連續包含「女鞋」的產品，「女款 Nike 跑步鞋」等名稱因「女」和「鞋」不相鄰而被漏掉
- **優化方案**：
  1. **中文拆字搜尋**：自動將中文搜尋詞拆成單字，用 AND 邏輯匹配（每個字都必須出現在名稱/描述/分類中至少一處）
  2. **英文按空格拆分**：「Nike Air」拆成 ["Nike", "Air"]，兩個詞都必須匹配
  3. **混合搜尋支援**：「Nike 女鞋」→ ["Nike", "女", "鞋"]，全部必須匹配
  4. 同時搜尋產品名稱、描述、分類名稱
- **使用範例**：
  - 搜尋「女鞋」→ 匹配所有名稱/描述含「女」且含「鞋」的產品（如「女款休閒鞋」「女士跑步鞋」）
  - 搜尋「男運動鞋」→ 匹配含「男」「運」「動」「鞋」的產品
  - 搜尋「Nike」→ 找到名稱含「Nike」的所有產品
- **技術變更**：`src/graphql/resolvers/productResolvers.ts` 搜尋邏輯、`app/search/page.tsx` 移除無效 gender 參數
- **效果**：搜尋結果從十幾筆提升到數百筆，大幅改善搜尋體驗
...
### 2026-02-01 ⚡ 限時搶購管理簡化（單一活動模式）

**更新時間**: 2026-02-01 | **優先級**: 🟢 架構簡化

#### 設計理念

限時搶購活動永遠只有一個，多組設計只會增加操作複雜度。因此簡化為：
- **單一表單編輯**：不再有活動列表、新增、刪除功能
- **使用 upsert 模式**：存在就更新，不存在就創建
- **啟用即首頁顯示**：開啟活動就會顯示在首頁

#### 後台操作

**限時搶購設定**（`/admin/homepage` → 限時搶購 Tab）
- 設定活動標題、描述
- 設定開始/結束時間
- 設定折扣百分比、顯示商品數量
- 選擇參與搶購的商品
...
- 簡化 GraphQL：使用 `upsertFlashSale` 替代 CRUD 操作
- `activeFlashSale` 查詢：返回唯一的活動記錄
- 資料庫：清理重複記錄，保持單一記錄

---

### 2026-01-13 ⚡ 限時搶購首頁設定功能（已簡化）

> ⚠️ 此功能已於 2026-02-01 簡化為單一活動模式，以下為歷史記錄

**更新時間**: 2026-01-13 | **優先級**: 🟢 功能增強

**原始設計**（已廢棄）：
- 支援創建多個限時搶購活動
- 可設定每個活動是否顯示在首頁（`showOnHomepage` 開關）
- 可設定首頁顯示排序順序（`sortOrder`）
